...
def solve_model(
    ...
    solver_kwargs["io_api"] = (
        "direct"  # default is 'lp', but 'direct' is faster and doesn't print to console
    )
...
def try_all_designs(
    scenarios: List[ScenarioData], settings: Dict, save_all: str = None
) -> EnergyModel:
    ...
    for techs in itertools.combinations(
        ...
        if settings["solver_settings"]["verbose"]:
            print(f"Designing system with technologies: {techs_str}")

        solved_model = solve_model(scenarios, settings)

        if save_all is not None:
            solved_model.save_results(
                os.path.join(save_all, f"{techs_str}_design.yaml")
            )
