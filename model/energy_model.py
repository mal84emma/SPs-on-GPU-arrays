...
class EnergyModel:
    ...
    def generate_SP(
        self, scenarios: Iterable[ScenarioData], settings: Dict, set_design: Dict = None
    ) -> Model:
        ...
        assert "storage_technologies" in settings.keys(), (
            "`storage_technologies` must be specified in settings (list of technologies used in specified system)."
        )
        ...
        if self.techs == ["none"]:
            ...
            assert len(self.techs) == settings["N_technologies"], (
                "Number of used storage technologies must match `N_technologies`."
            )
            assert all(
                [
                    tech in scenario.storage_technologies
        ...
        self.allow_elec_purchase = settings[
            "allow_elec_purchase"
        ]  # allow electricity purchase from grid
        self.grid_capacity = settings["grid_capacity"]  # maximum grid capacity (kW)
        self.solar_capacity_limit = settings[
            "solar_capacity_limit"
        ]  # maximum solar capacity (kWp)
        self.capex_budget = settings[
            "capex_budget"
        ...
        if self.use_CVaR:
            assert 0 < settings["CVaR_alpha"] < 0.5, (
                "CVaR_alpha must be between 0 and 0.5, as it is the right tail probability."
            )
        ...
        if set_design is not None:
            assert all(
                [key in set_design for key in ["wind_capacity", "solar_capacity"]]
            ), "Design dict must contain keys 'wind_capacity' and 'solar_capacity'."
            assert set_design["storage_technologies"] == self.techs, (
                "Storage technologies in set_design must match those in settings."
            )
        ...
        if all([scenario.probability is not None for scenario in scenarios]):
            self.scenario_weightings = np.array(
        ...
        storage_capacities = {
            tech: self.model.add_variables(lower=0, name=f"{tech}_capacity")
            for tech in self.techs
        }
        ...
        if set_design is None:
            self.model.add_constraints(
                solar_capacity,
                "<=",
                self.solar_capacity_limit,
                name="solar_capacity_limit",
            )
        else:  # set capacity variables to specified design values
            self.model.add_constraints(
                solar_capacity,
                "=",
                set_design["solar_capacity"]["value"],
                name="solar_capacity",
            )
            self.model.add_constraints(
                wind_capacity,
                "=",
                set_design["wind_capacity"]["value"],
                name="wind_capacity",
            )
            for tech in self.techs:
                self.model.add_constraints(
                    storage_capacities[tech],
                    "=",
                    set_design["storage_capacities"][tech]["value"],
                    name=f"{tech}_capacity",
                )
        ...
        for m, scenario in enumerate(scenarios):
            load = xr.DataArray(
                scenario.load[: self.T], coords=[pd.RangeIndex(self.T, name="time")]
            )
            wind = (
                xr.DataArray(
                    scenario.norm_wind_gen[: self.T],
                    coords=[pd.RangeIndex(self.T, name="time")],
                )
                * wind_capacity
            )
            solar = (
                xr.DataArray(
                    scenario.norm_solar_gen[: self.T],
                    coords=[pd.RangeIndex(self.T, name="time")],
                )
                * solar_capacity
            )
            elec_prices = xr.DataArray(
                scenario.elec_prices[: self.T],
                coords=[pd.RangeIndex(self.T, name="time")],
            )  # .clip(0)
            carbon_intensity = xr.DataArray(
                scenario.carbon_intensity[: self.T],
                coords=[pd.RangeIndex(self.T, name="time")],
            )
            ...
            for tech in self.techs:
                ...
                SOC = self.model.add_variables(
                    lower=0,
                    name=f"SOC_{tech}_s{m}",
                    coords=[pd.RangeIndex(self.T, name="time")],
                )
                Ein = self.model.add_variables(
                    lower=0,
                    name=f"Ein_{tech}_s{m}",
                    coords=[pd.RangeIndex(self.T, name="time")],
                )
                Eout = self.model.add_variables(
                    lower=0,
                    name=f"Eout_{tech}_s{m}",
                    coords=[pd.RangeIndex(self.T, name="time")],
                )
                ...
                self.model.add_constraints(
                    ...
                    "=",
                    0,
                    name=f"SOC_init_{tech}_s{m}",
                )
                self.model.add_constraints(
                    SOC[:-1]
                    - SOC[1:]
                    + np.sqrt(eta) * Ein[1:]
                    - 1 / np.sqrt(eta) * Eout[1:],
                    "=",
                    0,
                    name=f"SOC_series_{tech}_s{m}",
                )

                self.model.add_constraints(
                    Ein, "<=", P_max * self.delta_t, name=f"Pin_max_{tech}_s{m}"
                )
                self.model.add_constraints(
                    Eout, "<=", P_max * self.delta_t, name=f"Pout_max_{tech}_s{m}"
                )

                self.model.add_constraints(
                    SOC, "<=", storage_capacity, name=f"SOC_max_{tech}_s{m}"
                )
                self.model.add_constraints(
                    SOC, ">=", E_min, name=f"SOC_min_{tech}_s{m}"
                )

                battery_energy += Ein - Eout

            generation_curtailment = self.model.add_variables(
                lower=0,
                name=f"generation_curtailment_s{m}",
                coords=[pd.RangeIndex(self.T, name="time")],
            )
            self.model.add_constraints(
            ...
            supplied_energy = (
                -1 * (wind + solar - generation_curtailment) + battery_energy
            )
            ...
            if self.allow_elec_purchase:  # if grid import allowed
                self.model.add_constraints(
                    grid_energy,
                    "<=",
                    self.grid_capacity * self.delta_t,
                    name=f"pos_grid_limit_s{m}",
                )
            else:
                self.model.add_constraints(
                    grid_energy, "<=", 0, name=f"green_power_only_s{m}"
                )
            self.model.add_constraints(
                grid_energy,
                ">=",
                -1 * self.grid_capacity * self.delta_t,
                name=f"neg_grid_limit_s{m}",
            )

            pos_grid_energy = self.model.add_variables(
                lower=0,
                name=f"pos_grid_energy_s{m}",
                coords=[pd.RangeIndex(self.T, name="time")],
            )  # slack variable for carbon emissions
            self.model.add_constraints(
                pos_grid_energy, ">=", grid_energy, name=f"pos_grid_energy_s{m}"
            )
            ...
            for tech in self.techs:
                storage_cost += (
                    scenario.storage_costs[tech] / scenario.storage_lifetimes[tech]
                ) * storage_capacities[tech]
            self.scen_obj_contrs[m] = {
                "wind": (
                    scenario.wind_capex / scenario.wind_lifetime + scenario.wind_opex
                )
                * wind_capacity,
                "solar": (
                    scenario.solar_capex / scenario.solar_lifetime + scenario.solar_opex
                )
                * solar_capacity,
                "storage": storage_cost,
                "elec": supplied_energy
                @ elec_prices,  # electricity cost without plant usage (constants not allowed in objective)
            ...
            self.model.add_constraints(
                sum(
                    [
                        self.scen_obj_contrs[m][key]
                        for key in ["wind", "solar", "storage"]
                    ]
                ),
                "<=",
                self.capex_budget,
                name=f"capex_budget_s{m}",
            )
        ...
        if self.use_CVaR:  # add CVaR objective contribution & constraints
            ...
            etas = self.model.add_variables(
                lower=0,
                name="CVaR_slack",
                coords=[pd.RangeIndex(self.M, name="scenarios")],
            )

            for m in range(
                self.M
            ):  # add eta constraints per scenario (due to xarray datatype headaches)
                corrected_scenario_objective = (
                    self.scenario_objectives[m]
                    + self.scenarios[m].load[: self.T]
                    @ self.scenarios[m].elec_prices[: self.T]
                )
                self.model.add_constraints(
                    etas[m] + xi[0],
                    ">=",
                    corrected_scenario_objective,
                    name=f"CVaR_threshold_s{m}",
                )

            self.CVaR_obj_contribution = (
                xi + 1 / self.alpha * (self.scenario_weightings * etas).sum()
            )
    ...
    def solve(self, **kwargs):
        ...
        if self.use_CVaR:
            constant_correction_factor = 1 / (1 + self.n_upweight * self.alpha)
        else:
            constant_correction_factor = 1

        self.load_elec_cost = self.scenario_weightings @ [
            self.scenarios[m].load[: self.T] @ self.scenarios[m].elec_prices[: self.T]
            for m in range(self.M)
        ]
        self.corrected_objective = (
            self.model.objective.value
            + constant_correction_factor * self.load_elec_cost
        )
    ...
    def get_flared_energy(self):
        ...
        for m in range(self.M):
            ...
            for tech in self.techs:
                ...
                e2 = (
                    getattr(self.model.variables, f"Ein_{tech}_s{m}").solution
                    * getattr(self.model.variables, f"Eout_{tech}_s{m}").solution
                )

                Ein_dumps = getattr(
                    self.model.variables, f"Ein_{tech}_s{m}"
                ).solution.where(e2 > 0, 0)
                Eout_dumps = getattr(
                    self.model.variables, f"Eout_{tech}_s{m}"
                ).solution.where(e2 > 0, 0)
                net_energy_in = Ein_dumps - Eout_dumps
                net_energy_gain = (
                    np.sqrt(eta) * Ein_dumps - 1 / np.sqrt(eta) * Eout_dumps
                )
            ...
            self.energy_flares[f"s{m}"].update(
                {
                    ...
                    "generation_curtailment": self.model.variables[
                        f"generation_curtailment_s{m}"
                    ].solution,
                }
            )
        ...
        for m in range(self.M):
            ...
            for tech in self.techs:
                if self.model.variables[f"{tech}_capacity"].solution > 0:
                    ...
                    num_cycles = total_energy_flow / (
                        2 * self.model.variables[f"{tech}_capacity"].solution.values
                    )
    ...
    def save_results(self, fpath: str) -> Tuple[Dict]:
        ...
        solve_stats_dict = {
            ...
            "run_time": {"unit": "s", "value": self.run_time},
        ...
        design_dict = {
            ...
            "storage_capacities": {
                tech: {
                    "unit": "kWh",
                    "value": float(
                        self.model.variables[f"{tech}_capacity"].solution.values
                    ),
                }
                for tech in self.techs
            },
        ...
        overall_storage_cost = (
            float(
                self.scenario_weightings
                @ [
                    self.scen_obj_contrs[m]["storage"].solution.values
                    for m in range(self.M)
                ]
            )
            if self.techs
        ...
        overall_objective_dict = {
            ...
            "overall_wind_cost": float(
                self.scenario_weightings
                @ [
                    self.scen_obj_contrs[m]["wind"].solution.values
                    for m in range(self.M)
                ]
            ),
            "overall_solar_cost": float(
                self.scenario_weightings
                @ [
                    self.scen_obj_contrs[m]["solar"].solution.values
                    for m in range(self.M)
                ]
            ),
            "overall_storage_cost": overall_storage_cost,
            "overall_elec_cost": float(
                self.scenario_weightings
                @ [
                    self.scen_obj_contrs[m]["elec"].solution.values
                    for m in range(self.M)
                ]
            )
            + float(self.load_elec_cost),
            "overall_carbon_cost": float(
                self.scenario_weightings
                @ [
                    self.scen_obj_contrs[m]["carbon"].solution.values
                    for m in range(self.M)
                ]
            ),
        }
        if self.use_CVaR:
            overall_objective_dict["overall_CVaR_contr"] = float(
                self.CVaR_obj_contribution.solution.values
            )
            overall_objective_dict["note"] = (
                "Objective contributions do not sum to overall objective due to scaling used with CVaR formulation."
            )
        ...
        for m in range(self.M):
            ...
            scen_storage_cost = (
                float(self.scen_obj_contrs[m]["storage"].solution.values)
                if self.techs
                else 0.0
            )
            scen_cost_dict = {
        ...
        return (
            ...
            design_dict,
            overall_objective_dict,
            scenario_objective_contributions_dict,
        )
