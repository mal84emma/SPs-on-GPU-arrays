...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        available_technologies = list(
            settings["probability_settings"]["storage"].keys()
        )
        combos = [["none"]] + list(
            itertools.combinations(
                available_technologies, settings["model_settings"]["N_technologies"]
            )
        )
    ...
    prior_scenarios = [
        ScenarioData.from_file(os.path.join(prior_scenarios_dir, f"scenario_{i}.yaml"))
        for i in range(prob_settings["n_prior_samples"])
    ]

    # Set up Gurobi environment
    if settings["solver_settings"]["solver"] == "gurobi":
        settings["solver_settings"]["env"] = get_Gurobi_WLS_env(
            silence=not settings["solver_settings"]["verbose"]
        )
    ...
    if tech_combo is None:  # do design for all technology combinations together
        ...
        shutil.copy(
            os.path.join(save_dir, f"{best_techs}_design.yaml"),
            os.path.join(save_dir, "best_design.yaml"),
        )
        print(f"Finished prior design @ {get_current_time()}")
    else:  # just evaluate selected technology combination
        print(
            f"Starting prior design for tech combo {tech_combo} @ {get_current_time()}"
        )
        settings["model_settings"]["storage_technologies"] = list(tech_combo)
        solved_model = solve_model(prior_scenarios, settings)
        solved_model.save_results(
            os.path.join(save_dir, f"{tech_combo_str}_design.yaml")
        )
        print(
            f"Finished prior design for tech combo {tech_combo} @ {get_current_time()}"
        )
